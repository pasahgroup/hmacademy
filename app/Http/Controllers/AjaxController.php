...
//        return ['id' => Qs::hash($q->id), 'name' => $q->name];
        ...
        $lgaRegions = $this->loc->getAllRegions($country_id);
        return $data = $lgaRegions->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }
...
//        return ['id' => Qs::hash($q->id), 'name' => $q->name];
        ...
        $lgaCities = $this->loc->getAllCities($state_id);
        return $data = $lgaCities->map(function($q){
            return ['id' => $q->id, 'name' => $q->name];
        })->all();
    }
