...
use App\Repositories\LocationRepoData;
...
{
    protected $loc,$locData, $my_class, $user, $student;

   public function __construct(LocationRepoData $locData,LocationRepo $loc, MyClassRepo $my_class, UserRepo $user, StudentRepo $student)
   {
    ...
    public function create()
    {
        ...
        $data['countries'] = $this->loc->getAllCountries();
    ...
    public function store(StudentRecordCreate $req)
    {
       ...
       /* $ct = ($ct == 'J') ? 'JSS' : $ct;
        ...
        $data['surname'] =$req->surname;
         $data['tribe'] =$req->tribe;
         $data['parental_status'] =$req->parental_status;
          $data['education_level'] =$req->education_level;

        $data['user_type'] = 'student';
        $data['name'] = ucwords($req->name);
        $data['code'] = strtolower(Str::random(10));
        $data['password'] = Hash::make('student');
        $data['photo'] = Qs::getDefaultUserImage();
        $adm_no = $req->adm_no;
        $data['username'] = strtolower(Qs::getAppCode().'/'.$ct.'/'.$sr['year_admitted'].'/'.($adm_no ?: mt_rand(1000, 99999)));
...
//dd($data['sr']);
    ...
    public function edit($sr_id)
    {
        ...
        $data['parents'] = $this->user->getUserByType('parent');
        $data['dorms'] = $this->student->getAllDorms();
        $data['states'] = $this->loc->getAllRegions();
        $data['nationals'] = $this->loc->getAllNationals();
